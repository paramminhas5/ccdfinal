...
const CcdxSocialSponsor = () => {
  ...
  return (
    <main className="bg-electric-blue text-cream min-h-screen">
      ...
      {/* HERO */}
      <section className="bg-electric-blue border-b-4 border-ink py-16 md:py-24 lg:py-32">
        <div className="container">
          <p className="font-display text-lime text-lg sm:text-xl mb-6 tracking-wide">/ SPONSOR THE SERIES</p>
          <h1 className="font-display text-cream leading-[0.9] text-5xl sm:text-6xl md:text-8xl lg:text-9xl mb-8 chunk-shadow-text">
            BE PART OF<br />SOMETHING<br />DIFFERENT.
          </h1>
          <p className="text-cream/90 text-base sm:text-lg md:text-xl font-medium max-w-2xl mb-8">
          ...
          <div className="flex flex-wrap gap-3">
            {HERO_CHIPS.map((c) => (
              <span
                key={c}
                className="inline-block bg-electric-blue text-cream border-4 border-cream px-4 py-2 font-display text-sm sm:text-base"
              >
                {c}
              </span>
            ))}
      ...
      {/* OPPORTUNITY */}
      <section className="bg-cream text-ink border-b-4 border-ink py-16 md:py-24 lg:py-32 bg-grain">
        <div className="container grid lg:grid-cols-2 gap-10 lg:gap-16">
          <div>
            <p className="font-display text-magenta text-lg sm:text-xl mb-6 tracking-wide">/ THE OPPORTUNITY</p>
            <h2 className="font-display leading-[0.95] text-4xl sm:text-5xl md:text-6xl lg:text-7xl mb-8">
              A CROWD THAT<br />ACTUALLY CARES.
            </h2>
            <p className="text-ink/80 text-base sm:text-lg md:text-xl font-medium mb-5">
            ...
            <p className="text-ink/80 text-base sm:text-lg md:text-xl font-medium">
          ...
          <ul className="space-y-4 self-center w-full">
            {STATS.map((s) => (
              <li
                key={s.label}
                className={`${s.bg} ${s.fg} border-4 border-ink chunk-shadow p-4 sm:p-5 flex items-center justify-between gap-4`}
              >
                <span className="font-display uppercase text-sm sm:text-base tracking-wide">{s.label}</span>
                <span className="font-display text-base sm:text-lg md:text-xl text-right">{s.value}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      {/* SERIES */}
      <section className="bg-electric-blue text-cream border-b-4 border-ink py-16 md:py-24 lg:py-32">
        <div className="container">
          <p className="font-display text-lime text-lg sm:text-xl mb-6 tracking-wide">/ THE SERIES</p>
          <h2 className="font-display leading-[0.95] text-4xl sm:text-5xl md:text-6xl lg:text-7xl mb-12">
            THREE SHOWS.<br />ONE GRAND FINALE.
          </h2>
          <div className="grid md:grid-cols-3 gap-6 mb-8">
            {SHOWS.map((s) => (
              <div key={s.title} className={`${s.bg} ${s.fg} border-4 border-ink chunk-shadow p-6`}>
                <p className="font-display text-xs sm:text-sm mb-3 opacity-70">{s.tag}</p>
                <h3 className="font-display text-2xl sm:text-3xl mb-3 leading-tight">{s.title}</h3>
                <p className="font-display text-xs sm:text-sm mb-4 opacity-80">{s.sub}</p>
                <p className="font-medium text-base sm:text-lg">{s.date}</p>
                <p className="text-sm opacity-80 mt-1">{s.meta}</p>
              </div>
            ))}
          </div>
          <div className="bg-magenta text-cream border-4 border-ink chunk-shadow-lg p-6 sm:p-8 md:p-10 grid md:grid-cols-[1fr_auto] gap-6 items-center">
            <div>
              <p className="font-display text-xs sm:text-sm mb-3 opacity-80">SEASON FINALE · DATE TBA</p>
              <h3 className="font-display text-3xl sm:text-4xl md:text-5xl mb-4 leading-tight">
                MEGA — GRAND FORMAT SHOW
              </h3>
              <p className="text-cream/90 text-base sm:text-lg max-w-2xl">
            ...
            <div className="text-center md:text-right">
              <p className="font-display text-5xl sm:text-6xl md:text-7xl leading-none">2,000+</p>
              <p className="font-display text-xs sm:text-sm mt-2 opacity-80">Expected attendance</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-cream text-ink border-b-4 border-ink py-16 md:py-24 lg:py-32 bg-grain">
        <div className="container">
          <p className="font-display text-magenta text-lg sm:text-xl mb-6 tracking-wide">/ SPONSOR TIERS</p>
          <h2 className="font-display leading-[0.95] text-4xl sm:text-5xl md:text-6xl lg:text-7xl mb-6">
            SUPPORT A SHOW.<br />OR THE WHOLE THING.
          </h2>
          <p className="text-ink/80 text-base sm:text-lg md:text-xl font-medium mb-12 max-w-3xl">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {TIERS.map((t) => (
              <article key={t.title} className={`${t.bg} ${t.fg} border-4 border-ink chunk-shadow p-6 flex flex-col`}>
                <div className="text-4xl mb-3">{t.badge}</div>
                <p className="font-display text-xs sm:text-sm mb-2 opacity-80 uppercase">{t.eyebrow}</p>
                <h3 className="font-display text-2xl sm:text-3xl mb-2 leading-tight">{t.title}</h3>
                <p className="font-medium mb-5 opacity-90">{t.tagline}</p>
                ...
                <a
                  href={SPONSOR_MAILTO}
                  className="inline-block text-center bg-ink text-cream font-display text-base sm:text-lg px-5 py-3 border-4 border-ink hover:translate-x-1 hover:translate-y-1 transition-transform"
                >
                  ENQUIRE NOW →
                </a>
              </article>
      ...
      {/* WHO SHOULD SPONSOR */}
      <section className="bg-acid-yellow text-ink border-b-4 border-ink py-16 md:py-24 lg:py-32">
        <div className="container">
          <p className="font-display text-magenta text-lg sm:text-xl mb-6 tracking-wide">/ WHO SHOULD SPONSOR</p>
          <h2 className="font-display leading-[0.95] text-4xl sm:text-5xl md:text-6xl lg:text-7xl mb-12">
          ...
          <ul className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {SPONSOR_CATEGORIES.map((c) => (
              <li
                key={c.label}
                className="bg-cream text-ink border-4 border-ink chunk-shadow p-5 font-display text-base sm:text-lg flex items-center gap-3"
              >
      ...
      <section className="bg-cream text-ink border-b-4 border-ink py-16 md:py-24 lg:py-32 bg-grain">
        <div className="container">
          <p className="font-display text-magenta text-lg sm:text-xl mb-6 tracking-wide">/ WHAT YOU GET</p>
          <h2 className="font-display leading-[0.95] text-4xl sm:text-5xl md:text-6xl lg:text-7xl mb-6">
            MORE THAN A LOGO.
          </h2>
          <p className="text-ink/80 text-base sm:text-lg md:text-xl font-medium mb-12 max-w-3xl">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {PERKS.map((p) => (
              <div key={p.title} className="bg-electric-blue text-cream border-4 border-ink chunk-shadow p-6">
                <div className="text-3xl mb-3" aria-hidden>{p.emoji}</div>
                <h3 className="font-display text-xl sm:text-2xl mb-2">{p.title}</h3>
                <p className="text-cream/85 text-sm sm:text-base">{p.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* CTA */}
      <section className="bg-lime text-ink border-b-4 border-ink py-16 md:py-24 lg:py-32">
        <div className="container max-w-4xl">
          <p className="font-display text-magenta text-lg sm:text-xl mb-6 tracking-wide">/ LET'S TALK</p>
          <h2 className="font-display leading-[0.95] text-5xl sm:text-6xl md:text-7xl lg:text-8xl mb-8">
            READY TO<br />SPONSOR?
          </h2>
