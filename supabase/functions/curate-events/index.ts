...
async function extractWithAI(text: string, sourceUrl: string, source: string, lovableKey: string) {
  ...
  const sys = `You extract a SINGLE dance/electronic/underground music event in Bangalore/Bengaluru from a single event page. Today is ${today}. Only return the event if:
- it is a real bookable individual event (not a category/listing/venue homepage)
- it is in Bengaluru/Bangalore
- it is dance, electronic, techno, house, disco, club, or underground
